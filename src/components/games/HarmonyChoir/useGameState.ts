...
export function useGameState() {
  ...
  const [currentRole, setCurrentRole] = useState<VoicePart>('soprano');
  ...
  const switchRole = useCallback(() => {
    const currentIndex = voiceParts.indexOf(currentRole);
    const nextIndex = (currentIndex + 1) % voiceParts.length;
    setCurrentRole(voiceParts[nextIndex]);
  }, [currentRole]);
